...
{
    internal class Quest14_backup : BaseQuest
    {
        private const char RoundRock = 'O';
        private List<string> occurences = new List<string>();
        private List<long> cycleValues = new List<long>();
        public override Task Solve()
        {
            ...
            {
                string str = GetMd5(lines);
                long result2 = calcLoad(lines, n, m);
                if (occurences.Contains(str))
                {
                    int j = occurences.LastIndexOf(str);
                    int cycleLen = (i - j);

                    int q = (1000000000 - i) % cycleLen;

                    int nw = (q + j) % cycleLen;
                    long result3 = cycleValues[nw];
                    Console.WriteLine("Result3", result3);
                    break;
                }
                Console.WriteLine($"{i}/{1_000_000_000}");
                GoFullCycle(lines, n, m);
                cycleValues.Add(result2);
                occurences.Add(str);
            }
        ...
        private string GetMd5(char[][] lines)
        {
            string str = ConvertCharMatrixToString(lines);
        ...
        private string ConvertCharMatrixToString(char[][] mat)
        {
            ...
            for (int i = 0; i < mat.Length; i++)
                str = string.Join("", str, string.Join("", mat[i]));
            return str;
        }
